...
func main() {
	...
	for j := 0; j < 3; j++ {
		...
		// Reset highest probability field.
		board[maxi] = 0

		//		fmt.Println(maxi, ":", 100*float64(max)/float64(total))
		result = result + fmt.Sprintf("%v", maxi)
	}

	fmt.Println("084/If, instead of using two 6-sided dice, two 4-sided dice are used, find the six-digit modal string")
	fmt.Println(result)
